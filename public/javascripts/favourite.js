document.addEventListener('DOMContentLoaded', () => {
	...
	addToFavoriteButtons.forEach((button) => {
		button.addEventListener('click', async (event) => {
			...
			const isFavorite =
				button.getAttribute('data-in-favorites') === 'true';
			const icon = button.querySelector('i');
			const flashMessage = document.getElementById('flash-message');
			const flashMessageCarName = document.getElementById(
			...
			try {
				...
				if (response.ok) {
					...
					flashMessage.textContent =
						result.message || 'Failed to toggle favorite status';
				...
				flashMessage.textContent =
					'An error occurred while toggling the favorite status';
				flashMessage.classList.remove('bg-green-500'); // Remove success background
				flashMessage.classList.add('bg-red-500'); // Add error background
			}
